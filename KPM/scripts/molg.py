...
class MolGraph(object):
    ...
    def to_rdkit_mol(self):
        ...
        #except ValueError:
        # return None
        #Chem.rdmolops.FastFindRings(rd_mol)
        return rd_mol
    ...
    def perceive_smiles(self, atommap=True):
        ...
        #mol = pybel.readstring("inchi",).next()
        ## Use RDKit to convert back to Smiles
        mol_sanitized = Chem.MolFromInchi(inchi)
        #open('testx.inchi','w').writelines([inchi+'\n'])
        ##pym = pybel.readstring('inchi',inchi)
        ##pym.write('mol',filename='testx.mol',overwrite=True)
        ##os.system('obabel -ixyz '+fili+' -oinchi -F -O testx.inchi ')
        ##os.system('obabel -iinchi testx.inchi -osmi -O testx.smi')
        #os.system('obabel -iinchi testx.inchi -osmi -crI -O testx.smi')
        ##os.system('obabel -iinchi testx.inchi -osmi -cirh -O testx.smi')
        #mol_sanitized = Chem.MolFromSmiles(open('testx.smi','r').readline().split()[0].strip())
        #os.system('obabel -iinchi testx.inchi -omol --gen3D -O testx.mol')
        #print(pybel.outformats)
        #quit()
        #mol_sanitized = Chem.MolFromMolFile('testx.mol')
        ...
        try:
         ...
         print('MATCH ERROR')
         return None 
        if len(match) != mol_with_map.GetNumAtoms():
          return None 
        ...
        return Chem.MolToSmiles(mol_sanitized)
        #return Chem.MolToSmiles(mol_sanitized,isomericSmiles=True,canonical=True)
        #return Chem.MolToSmiles(mol_sanitized,isomericSmiles=True,allHsExplicit=True)
...
def read_xyz_file(path, with_energy=False):
    ...
    """
    with open(path) as f: # ok, so here you want to read the file line by line, hence strip()
        contents = [line.strip() for line in f]

    xyzs = []
    i = 0
    while i < len(contents): # as long as i is less than the length of the file.
        if not contents[i]:  # Empty line
            break
        else:
            natoms = int(contents[i]) # number of atoms is the integer number of lines in the file.
